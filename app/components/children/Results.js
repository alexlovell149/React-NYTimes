...
var Results = React.createClass({
  ...
  componentWillReceiveProps: function(nextProps){
    ...
    var myResults = nextProps.results.map(function(search, i){
      ...
      return 
      <div className="list-group-item" key={i}><a href={search.web_url} target="_blank">{search.headline.main}</a>
      <br />
      {search.pub_date}
      <br />
      <button type="button" className="btn btn-warning" style={{'float': 'right', 'marginTop': '-39px'}} onClick={boundClick}>Save</button>
      </div>
    });
